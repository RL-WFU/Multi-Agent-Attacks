...
def run_attack(arglist, threshold = 0, attack_rate = 1, test=False):
    """
    ...
    with marl_sess as sess:
        ...
        while True:
            ...
            if attacking and episode_step > 2:

                correct_action = temp_actions[corrupted_agent]
                next_states = []
                for i in range(5):
                    temp_actions[corrupted_agent] = i
                    action_inputs = np.concatenate([episode_acts[-2:], np.expand_dims(temp_actions, axis=0)], axis=0)

                    next_state = get_logits(transition_model, episode_obs[-3:], in_length, action_inputs, model_type="transition") #GET THIS IMPLEMENTED W MODEL
                    next_states.append(next_state)

                next_states = np.asarray(next_states)
                next_0_states = next_states[:, :, :cor_agent_in_length]
                gt_states = np.asarray(episode_obs[-2:])
                #gt_states = gt_states[:, corrupted_agent, :]
                if scenario == 'simple_spread':
                    gt_states = gt_states[:, :18]
                else:
                    gt_states = gt_states[:, 8:18]
                gt_states = np.reshape(gt_states, newshape=(-1, cor_agent_in_length))

                agent0_inputs = [np.concatenate([gt_states, next_0_states[i]], axis=0) for i in range(5)]

                #Should be shape [5, 5]
                next_agent0_probs = get_logits(cor_act_model, agent0_inputs, cor_agent_in_length, model_type="policy")
                #print("1 " + str(next_agent0_probs.shape))

                #Should be shape [5, 1]
                correct_agent_0_acts = [np.argmax(next_agent0_probs[i]) for i in range(len(next_agent0_probs))]
                correct_agent_0_acts = np.expand_dims(np.asarray(correct_agent_0_acts), axis=1)
                #print("2 " + str(correct_agent_0_acts.shape))

                #Should be shape [5, 3, 54]
                other_agents_inputs = [np.concatenate([np.reshape(episode_obs[-2:], newshape=(2, in_length)), next_states[i]], axis=0) for i in range(5)]
                other_agents_inputs = np.asarray(other_agents_inputs)
                #print("3 " + str(other_agents_inputs.shape))
                #Should be shape [5, 3, 1]
                gt_actions = np.asarray(episode_acts[-2:])
                gt_actions = np.expand_dims(gt_actions[:, corrupted_agent], axis=1)
                other_agents_action_inputs = [np.concatenate([gt_actions, np.expand_dims(correct_agent_0_acts[i], axis=0)], axis=0) for i in range(5)]
                other_agents_action_inputs = np.asarray(other_agents_action_inputs)
                #print("4 " + str(other_agents_action_inputs.shape))
                #Should be shape [5, 2, 5]
                other_agents_probs = get_logits(other_act_model, other_agents_inputs, in_length, other_agents_action_inputs, model_type="other_policy")
                other_agents_probs = np.reshape(other_agents_probs, newshape=(-1, 2, 5))
                #print("5 " + str(other_agents_probs.shape))
                ...
                for i in range(5):
                    ...
                    for j in range(2):
                        agent_kl = 0
                        for k in range(5):
                            log_term = np.log(other_agents_probs[correct_action, j, k] / other_agents_probs[i, j, k])
                            agent_kl += other_agents_probs[correct_action, j, k] * log_term
                        if j == 0:
                            adv_kl += agent_kl
                        else:
                            coop_kl += agent_kl
                ...
                if kl_divergences[int(worst_action)] > attack_threshold:
                    actions[corrupted_agent] = worst_action
                    episode_attacks += 1
                    label = 1
            """
            each action will produce a new next state
            each next state will decide different actions of the agents
            pick an action which produces a next state that induces actions most different
            from the one they would have taken given the policy action
            """
